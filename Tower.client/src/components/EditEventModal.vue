<template>
  <div class="modal text-dark" id="editEvent">
...
export default {
  setup() {
    ...
    return {
      ...
      async editEvent() {
        try {
          await eventService.editEvent(state.editable, AppState.activeEvent)
          Modal.getOrCreateInstance(document.getElementById("editEvent")).toggle()
          state.editable = {}
        } catch (error) {
